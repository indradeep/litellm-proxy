...
class LiteLLMResponsesTransformationHandler(CompletionTransformationBridge):
    ...
    @staticmethod
    def _reasoning_only_choice(
        pending_reasoning_item: Optional[Dict[str, Any]],
        reasoning_content: Optional[str],
        index: int,
    ) -> Optional[Any]:
        if pending_reasoning_item is None:
            return None
    ...
    def _convert_response_output_to_choices(  # noqa: PLR0915
        ...
        for item in output_items:
            if isinstance(item, ResponseReasoningItem) or getattr(item, "type", None) == "reasoning":
            ...
            elif isinstance(item, BaseModel) and handle_raw_dict_callback is not None:
                choice, index = handle_raw_dict_callback(item=item.model_dump(), index=index)
                if choice is not None:
                    choices.append(choice)
            else:
        ...
        reasoning_only_choice = LiteLLMResponsesTransformationHandler._reasoning_only_choice(
            pending_reasoning_item=pending_reasoning_item,
            reasoning_content=reasoning_content,
            index=index,
        )
        if not choices and reasoning_only_choice is not None:
            choices.append(reasoning_only_choice)
    ...
    def transform_response(  # noqa: PLR0915
        ...
        if (
            ...
            and raw_response.incomplete_details.reason == "max_output_tokens"
        ):
            if len(choices) == 0:
                from litellm.types.utils import Choices, Message
...
class OpenAiResponsesToChatCompletionStreamIterator(BaseModelResponseIterator):
    ...
    def _record_tool_args_delta(
        ...
        if not delta:
            return
        streamed_by_output_index[output_index] = streamed_by_output_index.get(output_index, "") + delta
    ...
    def translate_responses_chunk_to_openai_stream(  # noqa: PLR0915
        ...
        if event_type == "response.created":
            ...
            return ModelResponseStream(
                ...
                **OpenAiResponsesToChatCompletionStreamIterator._chat_chunk_metadata_from_responses_event(parsed_chunk),
            )
            ...
            if output_item.get("type") == "function_call":
                ...
                output_index = OpenAiResponsesToChatCompletionStreamIterator._tool_call_output_index(parsed_chunk)
                chat_tool_index = OpenAiResponsesToChatCompletionStreamIterator._chat_tool_call_index(
                    tool_call_chat_index_by_output_index,
                    output_index,
                )
            ...
            remaining_args = OpenAiResponsesToChatCompletionStreamIterator._remaining_tool_args(
            ...
            remaining_input = OpenAiResponsesToChatCompletionStreamIterator._remaining_tool_args(
